...
let zombiesKilled = 0,  lastDamageTime = 0;
let startTime = Date.now();
const walkKeys = new Set(['w', 'a', 's', 'd', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight']), sprintKey = 'Shift', jumpKey = ' ';
const damageCooldown = 2000, sprintDuration = 5000, sprintCooldown = 5000; 
...
function loop() {
    ...
    for(let heal of heals){
        ...
        if (distance(camera, heal.obj) < 3 && heal.available) {
            heal.collect();
            player_health = Math.min(player_health + 25, maxHealth);
            q4.setAttribute("value", `Quest #4: ${health_collected} / ${questTargets.health} health collected`);
            healthElement.setAttribute('value', `Health: ${player_health}`);
        }
    }
